...
void CHistogram::initUI(int maximumUpCount,QVector<QString> taskNameVector,QVector<int> taskCountVector,int nullCount)
{
    ...
    {
        *m_pBarSet<< maximumUpCount-1 ;  //上标上限，设置为range最大值-1 !!!
    }
    ...
    for(int i=1;i<=nullCount;i++)
    {
        strListDate.append("暂无"+QString::number(i));
    }
   ...
   /*for(int i = 6;i >= 0; i--) {                //6~0，共七个日期，往字符串列表添加，最终成为X轴
    ...
    pAxisY->setTickCount(maximumUpCount+1);  //y轴线的次数，包含0那一条横线!!!
    pAxisY->setLabelFormat("%.2f");          //y轴数据类型
    pAxisY->setRange(0,maximumUpCount);      //y轴的取值范围!!!
